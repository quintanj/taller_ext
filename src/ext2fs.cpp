...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	bgd = block_group(blockgroup_for_inode(inode_number));
	index = blockgroup_inode_index(inode_number);
	block_offset = index / inodes_per_block;
	mem_offset = (index % inodes_per_block) * sizeof(Ext2FSInode);
	lba = bgd->inode_table + block_offset; // bgd->inode_table: LBA block number of first inode table block

	read_block(lba, buf);
	memcpy(inode, &(buf[mem_offset]), sizeof(Ext2FSInode));
	return inode;
...
{
	unsigned int block_size = 1024 << _superblock->log_block_size;
	unsigned int entradas_por_bloque = block_size / sizeof(int);
	unsigned int bloque_relativo;
	unsigned int index;
	unsigned int res;
	unsigned char buf[block_size];

	if (block_number < 12){
		bloque_relativo = block_number;
		index = bloque_relativo;
		res = (inode->block)[index];
	} else if (block_number < 12 + entradas_por_bloque) {
		bloque_relativo = block_number - 12;
		index = bloque_relativo;
		read_block((inode->block)[12], buf);
		res = buf[index>>2];
	} else if (block_number < 12 + entradas_por_bloque + entradas_por_bloque * entradas_por_bloque){
		bloque_relativo = block_number - (12 + entradas_por_bloque);
		index = bloque_relativo / entradas_por_bloque;
		...
		bloque_relativo = block_number - (12 + entradas_por_bloque + entradas_por_bloque * entradas_por_bloque);
		index = bloque_relativo / (entradas_por_bloque * entradas_por_bloque);
		read_block((inode->block)[14], buf);
		res = buf[index>>2];
		index = (bloque_relativo % (entradas_por_bloque * entradas_por_bloque)) / entradas_por_bloque;
		read_block(res, buf);
		res = buf[index>>2];
		index = bloque_relativo % entradas_por_bloque;
		read_block(res, buf);
